...
function crafty:ADDON_LOADED()
	...
	do
		...
		instructions:SetText(CR_SEARCH)
	...
	self.frame.MaterialsButton:SetText(CR_MATS)
	self.frame.MaterialsButton:SetWidth(max(52, 20 + ({self.frame.MaterialsButton:GetRegions()})[1]:GetStringWidth()))
	...
	self.frame.LinkButton:SetText(CR_LINK_1)
	self.frame.LinkButton:SetWidth(max(52, 20 + ({self.frame.LinkButton:GetRegions()})[1]:GetStringWidth()))
	...
	searchBox:SetWidth(self.frame:GetWidth() - 35 - self.frame.MaterialsButton:GetWidth() - self.frame.LinkButton:GetWidth())
end
		...
		if getn(self.found) > 0 then
			...
			for i = 1, self.mode == CRAFT and CRAFTS_DISPLAYED or TRADE_SKILLS_DISPLAYED do
				...
				if i == 1 then
					...
					skillButton:SetText(CR_NO_RESULTS)
					skillButton:UnlockHighlight()
					skillButton:Show()
				else
					skillButton:Hide()
